...
const mapArgsToCacheKey = (state, activityIdentifier) =>
  activityIdentifier || ' '
...
export default createCachedSelector(
  ...
  (data, activityIdentifier) => {
    ...
    if (firstActivity && firstActivity.modelName) {
      const key = pluralize(
        `${firstActivity.modelName[0].toLowerCase()}${firstActivity.modelName.slice(
          1
        )}`
      )
      return selectEntityByKeyAndJoin({ data: { [key]: data[key] } }, key, {
